...
bool ELF_LoadSection(FILE* f, u32 srcaddr, u32 dstaddr, u32 len)
{
	...
	if(!f)
	{
		printf(".Bootrom: ERROR: Invalid Loaded ELF Has No Size!\n");
		return false;
	}
	...
	printf(".Bootrom: ELF %08X bytes copied to address %08X\n",BSWAP32(len),BSWAP32(dstaddr));
...
#pragma error("ELF_CreateFileStructure not modified for this OS!");
	...
	{
		printf("Error: dvdroot folder is %d bytes larger than 1.39GB. File system not setup!\n", TotalFileSize - (1024*1024*1024*1.39));
		return 0;
	}
	else
	{
	...
	printf("ELF File Data: Loading %d files to 0x%08X, Filenames at 0x%08X", FileCount,
...
//	FileNames = (char *)(&Mem_RAM[RAM_24MB]);
	for(; (u32)FileNames < (u32)&Mem_RAM[RAM_24MB]; FileNames+=4)
		*((u32 *)(FileNames)) = BSWAP32((*((u32 *)FileNames)));

	return 0x80000000 + DataPos;
}

bool ELF_Open(char* szFileName)
{
	u32 FSTStart;
	...
	printf(".Bootrom: ELF Load(): Opened: %s (%d bytes)\n", szFileName, flen );
	...
	if( (pEhdr->e_ident[0] != 0x7F) || (pEhdr->e_ident[1] != 0x45) ||	// 0x74 | 'E'
		...
		printf(".Bootrom: ERROR: This ELF File is NOT for IBM PPC Series Processors! (Gekko)\n");
		delete [] pElf;
		return false;
	}
	...
	printf(".Bootrom: ELF - entry:%X shnum:%X shoff:%X \n", pEhdr->e_entry, pEhdr->e_shnum, pEhdr->e_shoff);
	...
	for( i=0; i<pEhdr->e_shnum; i++ )
	{
		...
		case SHT_PROGBITS:
			if( pShdr->sh_flags & SHF_ALLOC )	// WRITE=1 | ALLOC=2 | EXECINSTR=4
			{
				...
				{
					for( int x=0; x<=pShdr->sh_size; x++ )	// +=2
						Memory_Write8(pShdr->sh_addr + x, pElf[pShdr->sh_offset+x]);
//						RAM[(pShdr->sh_addr&0xFFFFFF)+x] = pElf[pShdr->sh_offset+x];
						//WriteMem((pShdr->sh_addr+(x*2)), pElf[(pShdr->sh_offset+(x*2))], 2);
				}
			}
		break;
		...
		default:
			printf(".Bootrom: ELF File Unknown SecHead[%x].type:%X \n", i, pShdr->sh_type);
		break;
		}
	}
...
//	sprintf(ELFCurrentDir, "%s\\dvdroot", ELFCurrentDir);
	...
	//setup the files
	FSTStart = ELF_CreateFileStructure(szFileName);
