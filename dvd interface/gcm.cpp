...
char	CurrentGameName[992];
char	CurrentGameCRC[7];
...
DEFRealDVDRead(GCMDVDRead)
{
	...
	{
		printf("Reading invalid area of file!\n");
		return 0;
	}
...
*/
	...
	//0x400 - 0x20 = 3E0
	ReadFile(FileHandle, CurrentGameName, 0x3E0, &BytesRead, 0);

	if(DumpGCMBlockReads)
		WriteFile(DumpFileHandle, CurrentGameName, 0x3E0, &BytesRead, 0);

	printf("Loading [%.4s] %s\n", Header, CurrentGameName);
...
//	memcpy(MEMPTR32(FSTInfo.MemLocation + (FileCount * sizeof(GCMFST))), FileNames, TempData);
	...
	HLE_GetGameCRC(CurrentGameCRC, (u8 *)Header, BannerCRC);
	...
	ReadFile(FileHandle, &Mem_RAM[0x81200000 & RAM_MASK], BSWAP32(AppLoaderHeader[5]), &BytesRead, 0);

	if(DumpGCMBlockReads)
		WriteFile(DumpFileHandle, &Mem_RAM[0x81200000 & RAM_MASK], BSWAP32(AppLoaderHeader[5]), &BytesRead, 0);
