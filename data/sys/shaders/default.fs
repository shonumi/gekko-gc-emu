...
#define STAGE_RESULT(s) \
 ...
 \
    stage_result = (vec4(__FSDEF_COMBINER_COLOR_D_##s, __FSDEF_COMBINER_ALPHA_D_##s) + \
        ...
        (mix(vec4(__FSDEF_COMBINER_COLOR_A_##s, __FSDEF_COMBINER_ALPHA_A_##s), \
        vec4(__FSDEF_COMBINER_COLOR_B_##s, __FSDEF_COMBINER_ALPHA_B_##s), \
        vec4(__FSDEF_COMBINER_COLOR_C_##s, __FSDEF_COMBINER_ALPHA_C_##s)) + \
        vec4(stage.color_bias, stage.color_bias, stage.color_bias, stage.alpha_bias)))); \
 \
    __FSDEF_COMBINER_COLOR_DEST_##s = __FSDEF_CLAMP_COLOR_##s(stage.color_scale * \
        stage_result.rgb); \
    __FSDEF_COMBINER_ALPHA_DEST_##s = __FSDEF_CLAMP_ALPHA_##s(stage_result.a);
...
void main() {
    ...
    STAGE_RESULT(0);
#if __FSDEF_NUM_STAGES > 0
    STAGE_RESULT(1);
#endif
#if __FSDEF_NUM_STAGES > 1
    STAGE_RESULT(2);
#endif
#if __FSDEF_NUM_STAGES > 2
    STAGE_RESULT(3);
#endif
#if __FSDEF_NUM_STAGES > 3
    STAGE_RESULT(4);
#endif
#if __FSDEF_NUM_STAGES > 4
    STAGE_RESULT(5);
#endif
#if __FSDEF_NUM_STAGES > 5
    STAGE_RESULT(6);
#endif
#if __FSDEF_NUM_STAGES > 6
    STAGE_RESULT(7);
#endif
#if __FSDEF_NUM_STAGES > 7
    STAGE_RESULT(8);
#endif
#if __FSDEF_NUM_STAGES > 8
    STAGE_RESULT(9);
#endif
#if __FSDEF_NUM_STAGES > 9
    STAGE_RESULT(10);
#endif
#if __FSDEF_NUM_STAGES > 10
    STAGE_RESULT(11);
#endif
#if __FSDEF_NUM_STAGES > 11
    STAGE_RESULT(12);
#endif
#if __FSDEF_NUM_STAGES > 12
    STAGE_RESULT(13);
#endif
#if __FSDEF_NUM_STAGES > 13
    STAGE_RESULT(14);
#endif
#if __FSDEF_NUM_STAGES > 14
    STAGE_RESULT(15);
#endif
    frag_dest = __FSDEF_STAGE_DEST;
