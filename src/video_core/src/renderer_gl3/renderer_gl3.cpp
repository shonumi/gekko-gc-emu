...
RendererGL3::RendererGL3() {
    ...
    texture_cache_ = new TextureCache();
}
 ...
 */
void RendererGL3::AddTexture(int num, int width, int height, common::Hash64 hash, u8* data) {
    int set = (num & 4) >> 2;
    int index = num & 7;

    TextureCache::CacheEntry texture;

    texture.hash = hash;
    texture.address = gp::g_bp_regs.tex[set].image_3[index].get_addr();
    texture.width = width;
    texture.height = height;
    texture.last_frame_used = 0;
    texture.type = gp::kTextureType_Normal;
    texture.format = (gp::TextureFormat)gp::g_bp_regs.tex[set].image_0[index].format;

    glGenTextures(1, &texture.gl_handle);
    glBindTexture(GL_TEXTURE_2D, texture.gl_handle);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);

    texture_cache_->AddTexture(texture);
}
...
bool RendererGL3::BindTexture(common::Hash64 hash, int num) {
    ...
    if (texture_cache_->GetTexture(hash, texture)) {
        glBindTexture(GL_TEXTURE_2D, texture.gl_handle);
        return true;
    }
    return false;
