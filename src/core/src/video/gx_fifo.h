...
#define GP_OPCODE(name)				optype EMU_FASTCALL						\
...
typedef void				optype;
typedef optype				(EMU_FASTCALL *gptable)(cgxlist * gxlist);
...
namespace gx_fifo
{
	...
	// command processing
	u8 EMU_FASTCALL check_size();
	void EMU_FASTCALL command_parser(cgxlist * gxlist);
	void EMU_FASTCALL call_displaylist(u32 _addr, u32 _size);
