...
DWORD WINAPI PlayAFCStream( LPVOID lpParam ) {
	...
	int filesize = dvd::RealDVDGetFileSize(dvdfilehandle);
	...
	dvd::RealDVDRead(dvdfilehandle,(u32 *)buf,0x20);
	filesize-=0x20;
	...
	while (audioIsDone == false) { // While the thread is still alive
		...
		int amtread=dvd::RealDVDRead(dvdfilehandle,(u32*)buf,min(DECODEBUFSIZE/32*18,filesize));
		filesize-=amtread;
...
/*
	...
	streamfilehandle = dvd::RealDVDOpen(filepath);
...
HLE(StreamLibStop)
{
	...
	dvd::RealDVDClose(streamfilehandle);
	streamfilehandle=0;
