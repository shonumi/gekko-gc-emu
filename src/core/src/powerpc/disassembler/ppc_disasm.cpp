/* $VER: ppc_disasm.cpp V1.6 (14.06.2004)
 *
 * Disassembler module for the PowerPC microprocessor family
 * Copyright (c) 1998-2002  Frank Wille (phx)
 *               2003-2004  Andrey Shestakov (org)
 *
 * disasm.c is freeware and may be freely redistributed as long as
 ...
 *
 *
 * v1.6  (14.06.2004) monk
 *       dcbz_l added
 * v1.5  (28.03.2004) org
 *       final version, tested in emulator.
 * v1.4  (26.10.2003) org
...
{
  int d = (int)PPCGETD(in);
  int tbrr = (int)((PPCGETB(in)<<5)+PPCGETA(in));
  char *s = dp->operands;
  char x;
...
{
    struct PPCDisasm dp;
    static u32 ppcOpcode, ppcAddress;
    
    ppcOpcode = opcode;
    ppcAddress = curInstAddr;

    dp.opcode = opcodeStr;
    dp.operands = operandStr;
    dp.instr = (ppc_word *)&ppcOpcode;
    dp.iaddr = (ppc_word *)&ppcAddress;

    PPC_Disassemble(&dp);

    if(dp.type == BRANCH_OPCODE) *nextInstAddr = dp.displacement;
    else *nextInstAddr = curInstAddr + 4;
