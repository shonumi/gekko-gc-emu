...
void WIN_Framerate(void)
{
	...
	if(t - last > 1000)
	{
		...
		SetWindowText(wnd.hWnd, (LPCWSTR)str1);
	}
}
...
void WIN_LoadFromString(char _filename[])
{
	...
	printf("openging gmc %s", _filename);
	dvd::LoadGCM(_filename);
...
void WIN_OpenFile(void)
{
	...
	ROM.lpstrFilter = (LPCWSTR)Filter;
	ROM.lpstrCustomFilter = (LPTSTR)NULL;
	ROM.nMaxCustFilter = 0L;
	ROM.nFilterIndex = 1L;
	ROM.lpstrFile = (LPWSTR)File;
	ROM.nMaxFile = sizeof(File);
	ROM.lpstrFileTitle = (LPWSTR)FileTitle;
	ROM.nMaxFileTitle = sizeof(FileTitle);

	if(path) ROM.lpstrInitialDir = (LPCWSTR)path;
	else ROM.lpstrInitialDir = (LPCWSTR)".";
	
	ROM.lpstrTitle = (LPCWSTR)"Open GameCube Executable";
	ROM.Flags = OFN_ENABLESIZING|OFN_HIDEREADONLY|OFN_PATHMUSTEXIST|OFN_FILEMUSTEXIST;
	ROM.nFileOffset = 0;
	ROM.nFileExtension = 0;
	ROM.lpstrDefExt = (LPCWSTR)"*.gcm";
	ROM.lCustData = 0;
...
//	WIN_LoadFromString(ROM.lpstrFile);
	strcpy(LoadedGameFile, (const char *)ROM.lpstrFile);
	emu.status = EMU_LOAD;
		...
		case WM_COMMAND:
		{
			...
			case ID_EMULATION_START:
				...
				if(cpu->loaded)
				{
					...
					MessageBox(hWnd,
					  (LPCWSTR)"Please load a ROM before starting the CPU!",
					  (LPCWSTR)"Notice",NULL);
				break;
			...
			case ID_OPTIONS_DISABLEHLE:
				...
				WritePrivateProfileString((LPCWSTR)"Debug", (LPCWSTR)"Disable HLE Patches", (LPCWSTR)szbuff, (LPCWSTR)str1);
				break;
			...
			case ID_OPTIONS_DISABLEINIPATCHES:
				...
				sprintf(str1, "%sGekko.ini", ProgramDirectory);
				WritePrivateProfileString((LPCWSTR)"Debug", (LPCWSTR)"Disable INI Patches", (LPCWSTR)szbuff, (LPCWSTR)str1);
				break;
			...
			case ID_DEBUGOPTIONS_DISPLAYFRAMERATE:
				...
				else
				{
					...
					if(cpu->is_on)
					{
						...
						SetWindowText(hWnd, (LPCWSTR)str1);
					}
				}

				sprintf(szbuff, "%d", DisplayFPS);
				sprintf(str1, "%sGekko.ini", ProgramDirectory);
				WritePrivateProfileString((LPCWSTR)"System", (LPCWSTR)"Display FPS", (LPCWSTR)szbuff, (LPCWSTR)str1);
				break;
			...
			case ID_DEBUG_DUMPOPCODE0:
				...
				sprintf(str1, "%sGekko.ini", ProgramDirectory);
				WritePrivateProfileString((LPCWSTR)"Debug", (LPCWSTR)"Dump Opcode 0", (LPCWSTR)szbuff, (LPCWSTR)str1);
				break;
			...
			case ID_DEBUG_PAUSEONUNKNOWNOPCODE:
				...
				sprintf(str1, "%sGekko.ini", ProgramDirectory);
				WritePrivateProfileString((LPCWSTR)"Debug", (LPCWSTR)"Pause On Unknown Opcode", (LPCWSTR)szbuff, (LPCWSTR)str1);
				break;
			...
			case ID_DEBUG_DEBUGGER:
				if(cpu->loaded)
					...
					MessageBox(hWnd,
					  (LPCWSTR)"Please load a ROM before opening the Debugger!",
					  (LPCWSTR)"Notice",NULL);
				break;
			...
			case ID_DEBUGOPTIONS_DUMPBLOCKREADS:
				{
					...
					sprintf(str1, "%sGekko.ini", ProgramDirectory);
					WritePrivateProfileString((LPCWSTR)"Debug", (LPCWSTR)"Dump GCM Reads", (LPCWSTR)szbuff, (LPCWSTR)str1);
					break;
				}
...
bool WIN_Create( void )
{
	...
	wc.lpszClassName = (LPCWSTR)"Gekko";
	wc.style = CS_OWNDC | CS_VREDRAW | CS_HREDRAW;
	RegisterClass(&wc);

	wnd.hWnd = CreateWindow((LPCWSTR)"Gekko", (LPCWSTR)APP_NAME,
