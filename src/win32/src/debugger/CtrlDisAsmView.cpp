...
BOOL CopyTextToClipboard(HWND hwnd, TCHAR *text)
{
	OpenClipboard(hwnd);
	EmptyClipboard();
	HANDLE hglbCopy = GlobalAlloc(GMEM_MOVEABLE, (strlen((const char *)text) + 1) * sizeof(TCHAR)); 
	if (hglbCopy == NULL) 
	{ 
		CloseClipboard(); 
		return FALSE; 
	...
	strcpy((char *)lptstrCopy, (const char *)text); 
	lptstrCopy[strlen((const char *)text)] = (TCHAR) 0;    // null character 
...
void CtrlDisAsmView::onPaint(WPARAM wParam, LPARAM lParam)
{
	...
	HICON breakPoint = (HICON)LoadIcon(GetModuleHandle(0),(LPCWSTR)IDI_ZBRKPT);
	...
	for (i=-numRows; i<=numRows; i++)
	{
		...
		TCHAR *dis = (TCHAR*)debugger->disasm(address);
		TCHAR *dis2=(TCHAR*)strchr((char*)dis,'\t');
		if (dis2)
		{
			*dis2=0;
			dis2++;
			char *mojs=strstr((char*)dis2,"0x8");
			if (mojs)
			for (int i=0; i<8; i++)
			{
				bool found=false;
			...
			if (mojs)
			{
				...
				SetTextColor(hdc,0x000000);
			TextOut(hdc,198,rowY1,dis2,strlen((const char *)dis2));
		}

		SetTextColor(hdc,0x007000);
		TextOut(hdc,90,rowY1,dis,strlen((const char *)dis)); //disassembly

		std::string name;
		if(HLE_MapGetDebugSymbol(address, name)) //debug symbol
		{
			SetTextColor(hdc, 0xff0000);
			TextOut(hdc, 370, rowY1, (LPCWSTR)name.c_str(), name.size());
		}
		...
		if(((inst&0xfe000000)>>26) == 18 && (inst & 3) == 1) //bl
		{
			...
			if(HLE_MapGetDebugSymbol(to, name)) //debug symbol
			{
				...
				name = "-> " + name;
				TextOut(hdc, 420, rowY1, (LPCWSTR)name.c_str(), name.size());
			}
		}

		SetTextColor(hdc,0x0000FF);
		if (strlen((const char *)desc))
			TextOut(hdc,295,rowY1,(LPCWSTR)desc,strlen((const char *)desc));
