...
//#define DO_DVD_HACK
	...
	u32 dvdfilehandle = 0;
	dvdfilehandle = RealDVDOpen(filepath);
...
//	DVDFileInfo *dvdfileinfo = (DVDFileInfo*)(&RAM[GPR(4)&RAM_MASK]);
//	dvdfileinfo->length = BSWAP32(RealDVDGetFileSize(dvdfilehandle));
//	dvdfileinfo->callback = 0;
	Memory_Write32(GPR(4) + sizeof(DVDCommandBlock) + 4, RealDVDGetFileSize(dvdfilehandle));
...
//		DVDFileInfo *dvdfileinfo = (DVDFileInfo*)(&RAM[GPR(3)&RAM_MASK]);
		for(i = 0; i < (GPR(5) >> 2); i++)
		{
			RealDVDRead(dvdfilehandle, &InData, 4);
			Memory_Write32(GPR(4) + (i * 4), InData);
		}

		for(i = (i * 4); i < GPR(5); i++)
		{
			RealDVDRead(dvdfilehandle, &InData, 1);
