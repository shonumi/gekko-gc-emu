...
*/
	...
	CompileBlockNeeded.CodeBlock = &GekkoCPURecompiler::CompileInstruction;
...
//	pPC = (u32*)&RAM[ireg.PC & RAM_MASK];
	...
	printf(".CPU: Gekko Recompiler Opened.\n");
}
...
GekkoF GekkoCPURecompiler::DoBranchChecks()
{
	...
	if(PipeIsClient)
	{
		...
		if(pause)
		{
			printf("Prev CR: 0x%08X\n\n", iregBackup.CR);
			printf("Memory locations\n");
			printf("----------------\n");
			printf("GPR: 0x%08X\tCR: 0x%08X\tSPR: 0x%08X\tFPR:0x%08X\n", (u32)&ireg.gpr[0], (u32)CR, (u32)&ireg.spr[0], (u32)&ireg.fpr[0]);
			printf("XER_CARRY: 0x%08X\tXER_OVERFLOW: 0x%08X\tXER_SUMMARYOVERFLOW: 0x%08X\n", (u32)&XER_CARRY, (u32)&XER_OVERFLOW, (u32)&XER_SUMMARYOVERFLOW);
			printf("RAM: 0x%08X\t\tL2: 0x%08X\n", Mem_RAM, Mem_L2);
		}
