...
CtrlMemView::CtrlMemView(HWND _wnd)
{
	...
	font = CreateFont(16,0,0,0,FW_DONTCARE,FALSE,FALSE,FALSE,DEFAULT_CHARSET,
		OUT_DEFAULT_PRECIS,CLIP_DEFAULT_PRECIS,DEFAULT_QUALITY,DEFAULT_PITCH, "Andale Mono");
	curAddress=0;
	rowHeight=16;
	align=4;
	selecting=false;
...
void CtrlMemView::onPaint(WPARAM wParam, LPARAM lParam)
{
	...
	for (i=-numRows; i<=numRows; i++)
	{
		...
		case MV_NORMAL:
			...
			TextOut(hdc,90,rowY1,temp,strlen(temp));
			...
			TextOut(hdc,420,rowY1,temp,strlen(temp));
		break;
