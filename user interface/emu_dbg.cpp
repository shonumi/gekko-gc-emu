...
void RefreshGPR(void)
{
	...
	for( i=0; i<32; i++ )
		SetDlgItemText( dbgRegs.hWnd, IDS_R0+i, x8s(ireg_gpr(i)) );
}
...
void RefreshFPR(void)
{
	...
	for( i=0; i<32; i++ )
		SetDlgItemText( dbgRegs.hWnd, IDS_R0+i, fld(ireg_fpr_64(i, 0)) );
}
...
void RefreshFPR_PS0(void)
{
	...
	for( i=0; i<32; i++ )
		SetDlgItemText( dbgRegs.hWnd, IDS_R0+i, fld(ireg_fpr_32(i, 0)) );
}
...
void RefreshFPR_PS1(void)
{
	...
	for( i=0; i<32; i++ )
		SetDlgItemText( dbgRegs.hWnd, IDS_R0+i, fld(ireg_fpr_32(i, 1)) );
}
...
void RefreshFPR0Hex(void)
{
	...
	for( i=0; i<32; i++ )
	{
		...
		SetDlgItemText( dbgRegs.hWnd, IDS_R0+i, x8s(*(u32 *)&data) );
	}
}
...
void RefreshSPR(void)
{
	...
	SetDlgItemText( dbgRegs.hWnd, IDS_R0, x8s(ireg_PC()) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R1, x8s(ireg_spr(I_LR)) );
	for(i=0; i < 2; i++)
		SetDlgItemText( dbgRegs.hWnd, IDS_R2+i, x8s(ireg_spr(I_SRR0 + i)) );
	for(i=0; i < 4; i++)
		SetDlgItemText( dbgRegs.hWnd, IDS_R4+i, x8s(ireg_spr(272 + i)) );
	for(i=0; i < 3; i++)
		SetDlgItemText( dbgRegs.hWnd, IDS_R8+i, x8s(ireg_spr(I_HID0 + i)) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R11, x8s(ireg_spr(921)) );
	SetDlgItemText (dbgRegs.hWnd, IDS_R12, x8s(ireg_spr(I_DMAL)) );
	SetDlgItemText (dbgRegs.hWnd, IDS_R13, x8s(ireg_spr(I_DMAU)) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R14, x8s(ireg_spr(18)) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R15, x8s(ireg_spr(19)) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R16, x8s(ireg_MSR()) );	
	SetDlgItemText( dbgRegs.hWnd, IDS_R17, x8s(ireg_FPSCR()) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R18, x8s(ireg_CR()) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R19, x8s(ireg_spr(I_XER)) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R20, x8s(ireg_spr(I_CTR)) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R21, x8s(ireg_spr(I_DEC)) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R22, x8s(ireg_spr(282)) );
	SetDlgItemText( dbgRegs.hWnd, IDS_R23, x8s(ireg_spr(287)) );
}
...
void ChangeRegsTab(u32 NewTab)
{
	...
	switch(dbgRegsTab)
	{
		case 0:		//GPR
			for(i = 0; i < 32; i++)
				SetDlgItemText( dbgRegs.hWnd, IDC_REGLBL0 + i, GPRList[i]);
			RefreshGPR();
			break;
		...
		case 1:		//SPR
			for(i = 0; i < 32; i++)
			{
				SetDlgItemText( dbgRegs.hWnd, IDC_REGLBL0 + i, SPRList[i]);
				if(strlen(SPRList[i]) == 0)
					SetDlgItemText( dbgRegs.hWnd, IDS_R0 + i, "");
			}
			RefreshSPR();
			break;
		...
		case 2:		//FPR 0
			for(i = 0; i < 32; i++)
			{
				...
				SetDlgItemText( dbgRegs.hWnd, IDC_REGLBL0 + i, RegName);
			}
			RefreshFPR();
			break;
...
{
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQRSWST, x8sb(REGPI32(PI_INTSR) & PI_MASK_RSWST) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQHSP, x8sb(REGPI32(PI_INTSR) & PI_MASK_HSP) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQDEBUG, x8sb(REGPI32(PI_INTSR) & PI_MASK_DEBUG) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQCP, x8sb(REGPI32(PI_INTSR) & PI_MASK_CP) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQPEFINISH, x8sb(REGPI32(PI_INTSR) & PI_MASK_PEFINISH) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQPETOKEN, x8sb(REGPI32(PI_INTSR) & PI_MASK_PETOKEN) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQVI, x8sb(REGPI32(PI_INTSR) & PI_MASK_VI) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQMEM, x8sb(REGPI32(PI_INTSR) & PI_MASK_MEM) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQDSP, x8sb(REGPI32(PI_INTSR) & PI_MASK_DSP) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQAI, x8sb(REGPI32(PI_INTSR) & PI_MASK_AI) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQEXI, x8sb(REGPI32(PI_INTSR) & PI_MASK_EXI) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQSI, x8sb(REGPI32(PI_INTSR) & PI_MASK_SI) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQDI, x8sb(REGPI32(PI_INTSR) & PI_MASK_DI) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQRSW, x8sb(REGPI32(PI_INTSR) & PI_MASK_RSW) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQERROR, x8sb(REGPI32(PI_INTSR) & PI_MASK_ERROR) );

	SetDlgItemText( dbgIrq.hWnd, IDS_IRQRSWST2, x8sb(REGPI32(PI_INTMR) & PI_MASK_RSWST) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQHSP2, x8sb(REGPI32(PI_INTMR) & PI_MASK_HSP) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQDEBUG2, x8sb(REGPI32(PI_INTMR) & PI_MASK_DEBUG) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQCP2, x8sb(REGPI32(PI_INTMR) & PI_MASK_CP) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQPEFINISH2, x8sb(REGPI32(PI_INTMR) & PI_MASK_PEFINISH) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQPETOKEN2, x8sb(REGPI32(PI_INTMR) & PI_MASK_PETOKEN) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQVI2, x8sb(REGPI32(PI_INTMR) & PI_MASK_VI) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQMEM2, x8sb(REGPI32(PI_INTMR) & PI_MASK_MEM) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQDSP2, x8sb(REGPI32(PI_INTMR) & PI_MASK_DSP) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQAI2, x8sb(REGPI32(PI_INTMR) & PI_MASK_AI) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQEXI2, x8sb(REGPI32(PI_INTMR) & PI_MASK_EXI) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQSI2, x8sb(REGPI32(PI_INTMR) & PI_MASK_SI) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQDI2, x8sb(REGPI32(PI_INTMR) & PI_MASK_DI) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQRSW2, x8sb(REGPI32(PI_INTMR) & PI_MASK_RSW) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQERROR2, x8sb(REGPI32(PI_INTMR) & PI_MASK_ERROR) );

//	SetDlgItemText( dbgIrq.hWnd, IDS_VIINT0, x8s(REGVI32(VI_DI0)) );

//	SetDlgItemText( dbgIrq.hWnd, IDS_IRQPENDING, x8sb(cpu->gex_pending) );
//	SetDlgItemText( dbgIrq.hWnd, IDS_IRQEXTERNAL, x8sb(cpu->gex_external) );
	SetDlgItemText( dbgIrq.hWnd, IDS_IRQMSREE, x8sb(ireg_MSR() & MSR_BIT_EE) );
}

void RefreshDebugger()
{
	SetDlgItemText( dbg.hWnd, IDS_PC, x8s(ireg_PC()) );
	SetDlgItemText( dbg.hWnd, IDS_IC, x8s(ireg_IC()) );
	//SetDlgItemText( hDlg, IDS_LR, x8s(ireg.spr[8]) );//
	SetDlgItemText( dbg.hWnd, IDS_CR, x8s(ireg_CR()) );
	//SetDlgItemText( hDlg, IDS_CTR, x8s(ireg.spr[9]) );//
	//SetDlgItemText( hDlg, IDS_XER, x8s(ireg.spr[1]) );//
	SetDlgItemText( dbg.hWnd, IDS_MSR, x8s(ireg_MSR()) );
...
//			cpu->pPC =(u32*)&Mem_RAM[ireg_PC() & RAM_MASK];
		...
		case IDB_GOTO:
			...
			GetDlgItemText( hWnd, IDC_ADDR, szAddr, 8);
		...
		case IDB_SETPC: //(new)
			ZeroMemory(temp,128);
			GetWindowText(GetDlgItem(hWnd,IDC_EDITPC),temp,127);
...
//			cpu->pPC = (u32*)&Mem_RAM[ireg_PC() & RAM_MASK];
		...
		case IDB_SETCURSOR: //(new)
			GetDlgItemText( hWnd, IDC_EDITPC, szAddr, 9);
			dwAddr = ConvertFromHex(szAddr);
