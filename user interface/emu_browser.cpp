...
void BrowserCreateListView(void)
{
	...
	wnd.hWndList = CreateWindowEx( 0L,
		WC_LISTVIEW,
		"",
	...
	for( i = 0; i <= nFilter; i++ )
	{
		...
		if( hSearch != INVALID_HANDLE_VALUE )
		{
			while( GetLastError() != ERROR_NO_MORE_FILES )
			{
				...
				if(ReadGCMInfo(romList.filename, &romList.dwFileSize, Banner, Header))
				{
					...
					{
					case 0x45:
						ListView_SetItemText(wnd.hWndList, nCount, 2, "USA");
						break;

					case 0x4A:
						ListView_SetItemText(wnd.hWndList, nCount, 2, "JAP");
						break;

					default:
						ListView_SetItemText(wnd.hWndList, nCount, 2, "PAL");	  // Will be more precise later (with ini file ?)
						break;
					}
					...
					ListView_SetItemText(wnd.hWndList, nCount, 4, romList.comment);

					strcpy(romList.alttitle, romIni.title_alt);  
					if(strlen(romList.alttitle))
					{
						ListView_SetItemText(wnd.hWndList, nCount, 1, romList.alttitle);
					}else{
						ListView_SetItemText(wnd.hWndList, nCount, 1, romList.name);
					}

					wsprintf(szBuffer, "%iMbit", romList.dwFileSize / 131072);
					ListView_SetItemText(wnd.hWndList, nCount, 3, szBuffer);

					ListView_SetItemText(wnd.hWndList, nCount, 5, romList.filename);

					// Increment rom count
					nCount++;
				}else{
					MessageBox( wnd.hWnd, 
						"Unable to read ISO for rom browser!",
						"Error",
...
bool BrowserUpdateROMList(void)
{
	...
	// Set window title
	wsprintf(szBuffer, "%s - Refreshing Available ROM List", APP_NAME);
	SetWindowText(wnd.hWnd, szBuffer);

	// Scan for GCM
	BrowserScanPath(g_szRomPath);

	// Reset Window Title
	wsprintf( szBuffer, "%s", APP_NAME);
	SetWindowText( wnd.hWnd, szBuffer );
...
void BrowserSelectDir(void) 
{
	...
	if ((pidl = SHBrowseForFolder(&bi)) != NULL) {
		if (SHGetPathFromIDList(pidl, Directory)) {
			...
			WritePrivateProfileString("System","ROM Path", g_szRomPath, Buffer);
		}
	}
}