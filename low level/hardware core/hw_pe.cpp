...
{
	printf(".Flipper: Undefined PE_Read8: %08X!\n", addr);
	return 0;
}

void __fastcall PE_Write8(u32 addr, u32 data)
{
	printf(".Flipper: Undefined PE_Write8: %08X := %08X !\n", addr, data);
}
...
u16 __fastcall PE_Read16(u32 addr)
{
	...
	default:
		printf(".Flipper: Undefined PE_Read16: %08X!\n", addr);
		return 0;
	}
}
...
void __fastcall PE_Write16(u32 addr, u32 data)
{
	...
	case PE_TOKEN:
		...
		break;
	default:
		printf(".Flipper: Undefined PE_Write16: %08X := %08X !\n", addr, data);
		return;
	}
}
...
{
	printf(".Flipper: Undefined PE_Read32: %08X!\n", addr);
	return 0;
}

void __fastcall PE_Write32(u32 addr, u32 data)
{
	printf(".Flipper: Undefined PE_Write32: %08X := %08X !\n", addr, data);
}
